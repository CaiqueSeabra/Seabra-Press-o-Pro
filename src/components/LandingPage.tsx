...
import { Activity, Camera, TrendingUp, ShieldCheck, Heart, ArrowRight, Share2, FileDown, CheckCircle2, Plus } from 'lucide-react';
...
export function LandingPage({ onGetStarted, isInstallable, onInstall, isWebView }: Props) {
    ...
    <div className="min-h-screen bg-black text-white selection:bg-blue-500/30">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 glass-panel border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {isWebView && (
              <div className="hidden sm:flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 text-amber-500 px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-8">
          ...
          <div className="flex flex-col items-center justify-center gap-4 pt-4">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full">
              <button 
                onClick={onGetStarted}
                className="w-full sm:w-auto flex items-center justify-center gap-3 bg-white/10 text-white border border-white/10 px-10 py-5 rounded-3xl font-black text-lg hover:bg-white/20 active:scale-95 transition-all group"
              >
                Abrir no Navegador
              ...
              <button 
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-3 bg-white/5 text-zinc-400 border border-white/5 px-10 py-5 rounded-3xl font-bold text-lg hover:bg-white/10 transition-all"
              >
                Conhecer Recursos
              </button>
            </div>
      ...
      {/* Boss-Ready Section */}
      <section className="py-24 px-6 border-t border-white/5 bg-blue-600/5">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-500/30 text-[9px] font-black uppercase tracking-widest text-blue-400">
            <ShieldCheck className="w-3 h-3" />
            Certificado para Empresas e Clínicas
          </div>
          <h2 className="text-3xl sm:text-5xl font-black tracking-tight leading-none">
            Pronto para virar APP Oficial (.apk)
          </h2>
          <p className="text-zinc-400 font-medium leading-relaxed">
            Este sistema utiliza a tecnologia <strong>Trusted Web Activity (TWA)</strong> do Google. Ele é 100% compatível com a Play Store e pode ser convertido em um executável (.apk) em segundos.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
            <div className="p-6 glass-card border-blue-500/10">
              <h4 className="text-sm font-black mb-2 text-white uppercase tracking-wider">Vantagem Executiva</h4>
              <p className="text-xs text-zinc-500 leading-relaxed">Atualizações instantâneas sem precisar baixar novos arquivos. O chefe vê os dados em tempo real.</p>
            </div>
            <div className="p-6 glass-card border-blue-500/10">
              <h4 className="text-sm font-black mb-2 text-white uppercase tracking-wider">Segurança ISO</h4>
              <p className="text-xs text-zinc-500 leading-relaxed">Criptografia de ponta a ponta e conformidade com LGPD para dados sensíveis de saúde.</p>
            </div>
          </div>
        </div>
      </section>
